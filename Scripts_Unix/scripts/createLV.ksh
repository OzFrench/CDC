...
#               15/07/02                 #
#                 V3.1                   #
#   Script de création de raw device     #
##########################################
...
function genlist {
print "Sur quel Volume Group voulez vous créer le raw device?"
print " ---------------------------------------------------- "
...
while [[ $VG != [0-$n] ]]
 do
  print "Veuillez choisir le volume group sur lequel vous desirez créer le raw device!"
  read VG
 done
...
while [[ $CONFIRM != "oui" && $CHANGE != "oui" ]]
 do
  ...
  print ""
  print "Confirmez vous ces données? [oui/non]"
  print ""
...
if [[ $CONFIRM = "vi" ]]
 then
  clear
  print ""
  print "Bon, arrete tes bétises ou tu t'prepare des reveils penibles...!!!!"
  print ""
fi
  ...
  read PARAM
   ...
   case $PARAM in
    ...
    2)
     ...
     print ""
     print "Pour annuler la création d'un raw device, mettre la taille a 999!"
     printf "Nouveau nom (si ok tapez Entrée) : " 
     read RAWNAME
     if [[ $RAWNAME = "" ]]
      then
...
until (($numraw > $NBRRAW))
 do
  if ((${SIZE[$numraw]} == 0))
   then
    print ""
    print "Création du raw device \"${RAWNAME[$numraw]}\" annulée pour cause de taille nulle."
    numraw=$numraw+1
   else
    print "Création du raw device \"${RAWNAME[$numraw]}\" de ${SIZE[$numraw]} PP (${I[$numraw]} Mo) sur $VOLGROUP."
...
cat crrawdev.tmp | mail -s "Création d'un raw device sur `hostname`." root
